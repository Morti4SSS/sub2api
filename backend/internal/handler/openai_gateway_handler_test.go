...
func TestOpenAIResponses_APIKeyPassthroughPool5xxRetriesThenExhaustsMaxSwitches(t *testing.T) {
	...
	require.Equal(t, "Upstream service temporarily unavailable. Last upstream error: 502 temporary upstream failure", gjson.GetBytes(rec.Body.Bytes(), "error.message").String())
}
