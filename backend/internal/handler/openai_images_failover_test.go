...
func TestOpenAIGatewayHandlerImages_ServerErrorFailsOverAndReturnsClearErrorWhenExhausted(t *testing.T) {
	...
	require.Equal(t, "Upstream service temporarily unavailable. Last upstream error: 502 image backend unavailable", gjson.GetBytes(rec.Body.Bytes(), "error.message").String())
