...
const (
	...
	defaultOpenAITextTestPrompt  = "Respond with OK."
	defaultClaudeTextTestPrompt  = "hi"
)
...
func accountTextTestPrompt(prompt string, fallback string) string {
...
func createTestPayload(modelID string, prompt string) (map[string]any, error) {
	...
	testPrompt := accountTextTestPrompt(prompt, defaultClaudeTextTestPrompt)
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	probeSessionID := openAIResponsesProbeSessionID(account.ID)
...
func createOpenAITestPayload(modelID string, isOAuth bool, prompt ...string) map[string]any {
	...
	if len(prompt) > 0 {
		if value := strings.TrimSpace(prompt[0]); value != "" {
			testPrompt = value
		}
	}
	payload := map[string]any{
