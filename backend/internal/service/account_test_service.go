...
const (
	defaultClaudeTestPrompt      = "Please explain in two concise sentences how you would distinguish a successful model response, a missing upstream model, and an upstream rate-limit error."
	defaultOpenAITextTestPrompt  = "Please explain in two concise sentences how you would distinguish a successful model response, a missing upstream model, and an upstream rate-limit error."
...
func normalizeAccountTestPrompt(prompt, fallback string) (string, error) {
	...
	if utf8.RuneCountInString(normalized) < minimumAccountTestPromptCharacters {
		return "", fmt.Errorf("test prompt must contain at least %d characters", minimumAccountTestPromptCharacters)
	}
...
func createTestPayloadWithPrompt(modelID string, prompt string) (map[string]any, error) {
	...
	if testPrompt == "" {
		testPrompt = defaultClaudeTestPrompt
	}
...
func createOpenAITestPayload(modelID string, isOAuth bool) map[string]any {
	return createOpenAITestPayloadWithPrompt(modelID, isOAuth, defaultOpenAITextTestPrompt)
}
