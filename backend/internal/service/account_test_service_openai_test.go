...
`))
	...
	require.Equal(t, "probe_responses_89", req.Header.Get("Session_Id"))
	require.Equal(t, "probe_responses_89", req.Header.Get("Conversation_Id"))
	reqBody, readErr := io.ReadAll(req.Body)
	require.NoError(t, readErr)
	require.Equal(t, defaultOpenAITextTestPrompt, gjson.GetBytes(reqBody, "input.0.content.0.text").String())
