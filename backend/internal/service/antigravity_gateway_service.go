...
func (s *AntigravityGatewayService) TestConnection(ctx context.Context, account *Account, modelID string, prompt ...string) (*TestConnectionResult, error) {
	...
	if strings.HasPrefix(modelID, "gemini-") {
		requestBody, err = s.buildGeminiTestRequest(projectID, mappedModel, firstPromptArg(prompt))
	} else {
		requestBody, err = s.buildClaudeTestRequest(projectID, mappedModel, firstPromptArg(prompt))
	}
	if err != nil {
...
func firstPromptArg(prompt []string) string {
	if len(prompt) == 0 {
		return ""
	}
	return prompt[0]
...
func antigravityTestPrompt(prompt string) string {
		...
		"contents": []map[string]any{
			{
				...
				"parts": []map[string]any{
					{"text": antigravityTestPrompt(prompt)},
				},
			},
		},
...
func (s *AntigravityGatewayService) buildClaudeTestRequest(projectID, mappedModel string, prompt string) ([]byte, error) {
	claudeReq := &antigravity.ClaudeRequest{
		...
		Messages: []antigravity.ClaudeMessage{
			{
				Role:    "user",
				Content: json.RawMessage(strconv.Quote(antigravityTestPrompt(prompt))),
			},
		},
		MaxTokens: 1,
		Stream:    false,
