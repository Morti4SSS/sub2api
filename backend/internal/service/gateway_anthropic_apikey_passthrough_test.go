...
func TestGatewayService_AnthropicAPIKeyPassthrough_ClaudeCodeEffortMapping(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		{name: "count_tokens", endpoint: "count_tokens"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rec := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(rec)
			body := []byte(`{"model":"glm-5.1","output_config":{"effort":"xhigh"},"messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}],"max_tokens":1024}`)
			parsed := &ParsedRequest{
				Body:   NewRequestBodyRef(body),
				Model:  "glm-5.1",
				Stream: false,
			...
			account := &Account{
				ID:          104,
				Name:        "anthropic-apikey-pass-effort",
				Platform:    PlatformAnthropic,
				Type:        AccountTypeAPIKey,
				Concurrency: 1,
				Credentials: map[string]any{
			...
			require.Equal(t, "provider-glm-5.1", gjson.GetBytes(upstream.lastBody, "model").String())
