...
export const buildClaudeCodeEffortSummary = (entry: ClaudeCodeEffortForm) => {
  ...
  for (const level of ['low', 'medium', 'high', 'xhigh', 'max'] as const) {
    ...
    if (!mapped) continue
    groups.set(mapped, [...(groups.get(mapped) ?? []), level])
  }
...
export const readClaudeCodeCatalog = (extra?: Record<string, unknown>): ClaudeCodeCatalogForm[] => {
  ...
  if (!Array.isArray(raw)) {
    return [createEmptyClaudeCodeCatalogEntry()]
  }
  const entries = raw
    .map((item) => {
...
export const readClaudeCodeEffortMappings = (extra?: Record<string, unknown>): ClaudeCodeEffortForm[] => {
  ...
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) {
    return [createEmptyClaudeCodeEffortEntry()]
  }
...
export const writeClaudeCodeConfigToExtra = (
  ...
  const catalogPayload = catalog
    .map((entry) => {
      ...
      if (upstreamModel) {
        payload.upstream_model = upstreamModel
      }
      if (entry.supports1m) {
        payload.supports_1m = true
      }
  ...
  if (Object.keys(effortPayload).length > 0) {
    extra.claude_code_effort_mapping = effortPayload
  } else {
