  ...
  admin: {
    ...
    accounts: {
      ...
      openai: {
        ...
        compactAdvancedTest: 'Codex 高级测试',
        compactTestEnabled: '测试 /responses/compact 接口',
        compactTestHint: '仅检查 Compact 接口能力，不发送普通问答测试。',
      ...
      anthropic: {
        ...
        claudeCodeMapping: 'Claude Code 模型与思考映射',
        claudeCodeMappingDesc:
          '通用配置：让 Claude Code /model 显示映射模型，并把 low/medium/high/xhigh/max 折叠到上游实际支持的强度字段和值。',
        claudeCodeRole: '角色',
        claudeCodeDisplayName: '显示名',
        claudeCodeRequestModel: 'Claude Code 请求模型',
        claudeCodeUpstreamModel: '上游实际模型',
        claudeCodeSupports1m: '支持 1M 上下文',
        claudeCodeCapabilities: '能力标签',
        claudeCodeModelPreview: '/model 显示 {visible}；请求 {requestModel}；转发 {upstream}',
        claudeCodeAddModel: '添加模型映射',
        claudeCodeSyncModels: '同步上游模型',
        claudeCodeSyncModelsLoading: '同步中...',
        claudeCodeSyncModelsSuccess: '已读取 {count} 个上游模型，可在真实模型输入框中选择',
        claudeCodeSyncModelsFailed: '同步上游模型失败，仍可手工填写真实模型名',
        claudeCodeModelSyncAdvanced: '模型同步高级设置',
        claudeCodeModelsURL: '自定义模型列表接口地址',
        claudeCodeModelsURLPlaceholder: '留空时根据 Base URL 自动使用 /v1/models',
        claudeCodeEffortMapping: '思考强度映射',
        claudeCodeEffortModel: '适用请求模型',
        claudeCodeEffortTarget: '上游目标字段',
        claudeCodeEffortSummary: '档位折叠：{summary}',
        claudeCodeAddEffort: '添加强度映射',
        claudeCodeContextWindow: '上下文窗口',
        claudeCodeContextPreset: '快捷值',
        claudeCodeThinkingEnabled: '映射思考强度',
        claudeCodeThinkingMode: '映射模式',
        claudeCodeThinkingModeLevels: '档位型',
        claudeCodeThinkingModeBudget: 'Token 预算型',
        claudeCodeOrderedLevels: '上游档位（从弱到强）',
        claudeCodeOrderedBudgets: 'Token 预算（从低到高）',
        claudeCodeOverrides: '逐档覆盖',
        claudeCodeAdvancedSettings: '高级设置',
      },
      ...
      sendingTestMessage: '发送测试问题："{prompt}"',
      ...
      testModel: '测试模型',
      testPrompt: '完整问题测试',
      testPromptLabel: '测试问题',
      testPromptPlaceholder: '请输入一个完整问题',
      testPromptHint: '用于真实问答测试；请使用至少 24 个字符的完整问题，不要只发送 hi、hello、你好、ping、test 等打招呼或探活词。',
      defaultConnectionTestPrompt: '请用两句话说明：调用第三方模型接口时，如何区分模型可用、模型不存在和上游限流这三种结果？',
      testPromptProbeRejected: '测试问题至少需要 24 个字符，且不能只是 hi、hello、你好、ping、test 等打招呼或探活词。',
      testDiagnosticAccount: '固定账号',
      testDiagnosticClient: '客户端身份',
      testDiagnosticGateway: '网关路径',
      testDiagnosticModel: '模型映射',
